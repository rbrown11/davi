    ...
    if i != 0:
        ...
        if previous_detection == "ind3_changed_id":
            ...
            ind3_abx_p = ant3_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'x')][i-1]
            ind3_aby_p = ant3_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'y')][i-1]

        distance_ind3_ind3 = abs(davi.find_distance(
            ind3_abx, ind3_abx_p, ind3_aby, ind3_aby_p))

        if math.isnan(ind3_abx) or math.isnan(ind3_aby):
            test2 = list(new_row_ant3)
            ant3_copy.loc[i] = test2

        elif math.isnan(ind3_abx_p) or math.isnan(ind3_aby_p):

            for j in reversed(range(0, i)):

                if previous_detection == "ind3_changed_id":
                    j_ind3_abx = ant3_copy[(
                        'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'x')][j]
                else:
                ...
                if math.isnan(j_ind3_abx):
                    ...
                    if previous_detection == "ind3_changed_id":
                        ...
                        j_ind3_abx = ant3_copy[(
                            'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'x')][j]
                        j_ind3_aby = ant3_copy[(
                            'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'y')][j]
            ...
            if j != 0:
                distance_ind3_ind3 = abs(davi.find_distance(
                    ind3_abx, j_ind3_abx, ind3_aby, j_ind3_aby))

            else:
                distance_ind3_ind3 = 11
#                test2 = list(new_row_ant3)
#                ant3_copy.loc[i] = test2

        if distance_ind3_ind3 < 10:
            test2 = list(new_row_ant3)
            ant3_copy.loc[i] = test2

        elif distance_ind3_ind3 > 10:
            ...
            if math.isnan(ind1_abx_p) or math.isnan(ind1_aby_p):
                ...
                if k != 0:
                    ...
                    if math.isnan(ind3_abx):
                        ...
                        distance_ind3_ind1_original = abs(davi.find_distance(
                            j_ind3_abx, j_ind1_abx_p, j_ind3_aby, j_ind1_aby_p))

                    else:

                        distance_ind3_ind1_original = abs(davi.find_distance(
                            ind3_abx, j_ind1_abx_p, ind3_aby, j_ind1_aby_p))

                else:
                    distance_ind3_ind1_original = 1000

            else:
                distance_ind3_ind1_original = abs(davi.find_distance(
                    ind3_abx, ind1_abx_p, ind3_aby, ind1_aby_p))
            ...
            if math.isnan(ind2_abx_p) or math.isnan(ind2_aby_p):
                ...
                if k != 0:
                    ...
                    if math.isnan(ind3_abx):
                        ...
                        distance_ind3_ind2_original = abs(davi.find_distance(
                            j_ind3_abx, j_ind2_abx_p, j_ind3_aby, j_ind2_aby_p))

                    else:

                        distance_ind3_ind2_original = abs(davi.find_distance(
                            ind3_abx, j_ind2_abx_p, ind3_aby, j_ind2_aby_p))

                else:
                    distance_ind3_ind2_original = 1000

            else:
                distance_ind3_ind2_original = abs(davi.find_distance(
                    ind3_abx, ind2_abx_p, ind3_aby, ind2_aby_p))
            ...
            if math.isnan(ind4_abx_p) or math.isnan(ind4_aby_p):
                ...
                if k != 0:
                    ...
                    if math.isnan(ind3_abx):
                        ...
                        distance_ind3_ind4 = abs(davi.find_distance(
                            j_ind3_abx, j_ind4_abx_p, j_ind3_aby, j_ind4_aby_p))

                    else:

                        distance_ind3_ind4 = abs(davi.find_distance(
                            ind3_abx, j_ind4_abx_p, ind3_aby, j_ind4_aby_p))

                else:
                    distance_ind3_ind4 = 1000

            else:
                distance_ind3_ind4 = abs(davi.find_distance(
                    ind3_abx, ind4_abx_p, ind3_aby, ind4_aby_p))
            ...
            if math.isnan(ind5_abx_p) or math.isnan(ind5_aby_p):
                ...
                if k != 0:
                    ...
                    if math.isnan(ind3_abx):
                        ...
                        distance_ind3_ind5 = abs(davi.find_distance(
                            j_ind3_abx, j_ind5_abx_p, j_ind3_aby, j_ind5_aby_p))

                    else:

                        distance_ind3_ind5 = abs(davi.find_distance(
                            ind3_abx, j_ind5_abx_p, ind3_aby, j_ind5_aby_p))

                else:
                    distance_ind3_ind5 = 1000

            else:
                distance_ind3_ind5 = abs(davi.find_distance(
                    ind3_abx, ind5_abx_p, ind3_aby, ind5_aby_p))
            ...
            if math.isnan(ind6_abx_p) or math.isnan(ind6_aby_p):
                ...
                if k != 0:
                    ...
                    if math.isnan(ind3_abx):
                        ...
                        distance_ind3_ind6 = abs(davi.find_distance(
                            j_ind3_abx, j_ind6_abx_p, j_ind3_aby, j_ind6_aby_p))

                    else:

                        distance_ind3_ind6 = abs(davi.find_distance(
                            ind3_abx, j_ind6_abx_p, ind3_aby, j_ind6_aby_p))

                else:
                    distance_ind3_ind6 = 1000

            else:
                distance_ind3_ind6 = abs(davi.find_distance(
                    ind3_abx, ind6_abx_p, ind3_aby, ind6_aby_p))

            other_ant_distances = {"ind3_ind3": distance_ind3_ind3,
                                   "ind3_ind4": distance_ind3_ind4,
                                   "ind3_ind5": distance_ind3_ind5,
                                   "ind3_ind6": distance_ind3_ind6,
                                   "ind3_ind2": distance_ind3_ind2_original,
                                   "ind3_ind1": distance_ind3_ind1_original
                                   }
            ...
            if optimal_match == "ind3_ind1":

                test2 = list(new_row_ant1)
                ant3_copy.loc[i] = test2

                previous_detection = "ind3_changed_id"

            if optimal_match == "ind3_ind2":
            ...
            if optimal_match == 'ind3_ind3':

                test = list(new_row_ant3)
                ant3_copy.loc[i] = test

            if optimal_match == 'ind3_ind4':

                test = list(new_row_ant4)
                ant3_copy.loc[i] = test
                previous_detection = "ind3_changed_id"

            if optimal_match == 'ind3_ind5':

                test = list(new_row_ant5)
                ant3_copy.loc[i] = test
                previous_detection = "ind3_changed_id"

            if optimal_match == 'ind3_ind6':
...
for i in tqdm(range(0, len(data))):
    ...
    if i != 0:
        ...
        if previous_detection == "ind3_changed_id":
            ...
            ind3_abx_p = ant4_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind4', 'abdomen', 'x')][i-1]
            ind3_aby_p = ant4_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind4', 'abdomen', 'y')][i-1]
...
#                ant4_copy.loc[i] = test2
        ...
        if distance_ind3_ind3 < 10:
            ...
            if math.isnan(ind1_abx_p) or math.isnan(ind1_aby_p):
                ...
                distance_ind3_ind1_original = abs(davi.find_distance(
                    ind3_abx, ind1_abx_p, ind3_aby, ind3_aby_p))
...
for i in tqdm(range(0, len(data))):
    ...
    if i != 0:
        ...
        if previous_detection == "ind3_changed_id":
            ...
            ind3_abx_p = ant5_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind5', 'abdomen', 'x')][i-1]
            ind3_aby_p = ant5_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind5', 'abdomen', 'y')][i-1]
...
for i in tqdm(range(0, len(data))):
    ...
    if i != 0:
        ...
        if previous_detection == "ind3_changed_id":
            ...
            ind3_abx_p = ant6_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind6', 'abdomen', 'x')][i-1]
            ind3_aby_p = ant6_copy[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind6', 'abdomen', 'y')][i-1]
...
df2 = pd.concat([ant3_copy, ant4_copy, ant5_copy,
                ant6_copy], axis=1, join='inner')
df2.to_hdf('stage1_ant3.h5', key="changed_names", format="fixed")


print("ant 3 detections and comparisons complete")
...
ant3_copy = df2.xs('ind3', level='individuals', axis=1, drop_level=False)
...
ant3_ant1_mix = []
refined_ant3_ant1_mix = []
ant3_ant2_mix = []
refined_ant3_ant2_mix = []


refined_ant3_ant4_mix = []
refined_ant3_ant5_mix = []
refined_ant3_ant6_mix = []
...
for i in range(0, len(df2)):
    ...
    ind3_abx = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind3', 'abdomen', 'x')][i]
    ind3_aby = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind3', 'abdomen', 'y')][i]

    ind4_abx = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind4', 'abdomen', 'x')][i]
    ind4_aby = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind4', 'abdomen', 'y')][i]

    ind5_abx = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind5', 'abdomen', 'x')][i]
    ind5_aby = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind5', 'abdomen', 'y')][i]

    ind6_abx = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind6', 'abdomen', 'x')][i]
    ind6_aby = df2[('DLC_dlcrnetms5_full-modelJan10shuffle1_100000',
                    'ind6', 'abdomen', 'y')][i]
    ...
    if math.isnan(ind3_abx):
        ...
        if ind3_abx == ind1_abx:
            if math.isnan(ind3_abx):
                continue
            else:
                ant3_ant1_mix.append(i)

        if ind3_abx == ind2_abx:
            if math.isnan(ind3_abx):
                continue
            else:
                ant3_ant2_mix.append(i)

        elif ind3_abx == ind4_abx:
            if math.isnan(ind3_abx):
                continue
            else:
                ant3_ant4_mix.append(i)

        elif ind3_abx == ind5_abx:
            if math.isnan(ind3_abx):
                continue
            else:
                ant3_ant5_mix.append(i)

        elif ind3_abx == ind6_abx:
            if math.isnan(ind3_abx):
                continue
            else:
                ant3_ant6_mix.append(i)


if len(ant3_ant1_mix) > 0:
    refined_ant3_ant1_mix.append(ant3_ant1_mix[0])

if len(ant3_ant2_mix) > 0:
    refined_ant3_ant2_mix.append(ant3_ant2_mix[0])


if len(ant3_ant4_mix) > 0:
    refined_ant3_ant4_mix.append(ant3_ant4_mix[0])
if len(ant3_ant5_mix) > 0:
    refined_ant3_ant5_mix.append(ant3_ant5_mix[0])
if len(ant3_ant6_mix) > 0:
    refined_ant3_ant6_mix.append(ant3_ant6_mix[0])


for x, y in zip(ant3_ant1_mix[::], ant3_ant1_mix[1::]):
    if abs(x-y) > 1:
        refined_ant3_ant1_mix.append(x)


for x, y in zip(ant3_ant2_mix[::], ant3_ant2_mix[1::]):
    if abs(x-y) > 1:
        refined_ant3_ant2_mix.append(x)

for x, y in zip(ant3_ant4_mix[::], ant3_ant4_mix[1::]):
    if abs(x-y) > 1:
        refined_ant3_ant4_mix.append(x)


for x, y in zip(ant3_ant5_mix[::], ant3_ant5_mix[1::]):
    if abs(x-y) > 1:
        refined_ant3_ant5_mix.append(x)

for x, y in zip(ant3_ant6_mix[::], ant3_ant6_mix[1::]):
    if abs(x-y) > 1:
        refined_ant3_ant6_mix.append(x)


if len(ant3_ant1_mix) > 0:
    refined_ant3_ant1_mix.append(ant3_ant1_mix[-1])
if len(ant3_ant2_mix) > 0:
    refined_ant3_ant2_mix.append(ant3_ant2_mix[-1])

if len(ant3_ant4_mix) > 0:
    refined_ant3_ant4_mix.append(ant3_ant4_mix[-1])
if len(ant3_ant5_mix) > 0:
    refined_ant3_ant5_mix.append(ant3_ant5_mix[-1])
if len(ant3_ant6_mix) > 0:
    refined_ant3_ant6_mix.append(ant3_ant6_mix[-1])


if len(refined_ant3_ant1_mix) != 0:
    refined_ant3_ant1_mix = list(set(refined_ant3_ant1_mix))

if len(refined_ant3_ant2_mix) != 0:
    refined_ant3_ant2_mix = list(set(refined_ant3_ant2_mix))


if len(refined_ant3_ant4_mix) != 0:
    refined_ant3_ant4_mix = list(set(refined_ant3_ant4_mix))
if len(refined_ant3_ant5_mix) != 0:
    refined_ant3_ant5_mix = list(set(refined_ant3_ant5_mix))
if len(refined_ant3_ant6_mix) != 0:
    refined_ant3_ant6_mix = list(set(refined_ant3_ant6_mix))


previous_detection = "first"
full_ids = ['ind3_ind3']

for mix in refined_ant3_ant1_mix, refined_ant3_ant2_mix, refined_ant3_ant4_mix, refined_ant3_ant5_mix, refined_ant3_ant6_mix:
    ...
    if len(mix) != 0:
        ...
        for index, i in enumerate(mix):
            ...
            ind3_abx = data[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'x')][frame_id]
            ind3_aby = data[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'y')][frame_id]

            ind3_thx = data[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'thorax', 'x')][frame_id]
            ind3_thy = data[(
                'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'thorax', 'y')][frame_id]
            ...
            if ind3_theta == 0:
                ...
                if previous_detection == "ind3_changed_id":
                    ...
                    ind3_abx_p = ant3_copy[(
                        'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'x')][frame_id-1]
                    ind3_aby_p = ant3_copy[(
                        'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'abdomen', 'y')][frame_id-1]

                    ind3_thx_p = ant3_copy[(
                        'DLC_dlcrnetms5_full-modelJan10shuffle1_100000', 'ind3', 'thorax', 'x')][frame_id-1]
                    ind3_thy_p = ant3_copy[(
                ...
                ind3_theta_p = davi.get_vector_angle(
                    ind3_abx_p, ind3_aby_p, ind3_thx_p, ind3_thy_p)
                ...
                if ind3_theta_p == 0:
                    ...
                    if r != 0:
                        ...
                        ind3_waffle_p = davi.create_waffle(
                            ind3_abx_p, ind3_aby_p, ind3_theta_p, pix)
                    else:
                        ind3_waffle_p = davi.create_waffle(
                            ind3_abx, ind3_aby, ind3_theta, pix)
                ...
                if ind3_theta != 0:
                    ...
                    ind3_ind3 = davi.subtract_colours_2(
                        ind3_waffle, ind3_waffle_p)

                    if ind2_theta != 0:
                        ind2_waffle = davi.create_waffle(
                            ind2_abx, ind2_aby, ind2_theta, pix)
                        ind3_ind2 = davi.subtract_colours_2(
                            ind3_waffle, ind2_waffle)
                    else:
                        ind3_ind2 = 30
                    ...
                    if ind4_theta != 0:
                        ...
                        ind3_ind4 = davi.subtract_colours_2(
                            ind3_waffle, ind4_waffle)
                    else:
                        ind3_ind4 = 30
                    ...
                    if ind5_theta != 0:
                        ...
                        ind3_ind5 = davi.subtract_colours_2(
                            ind3_waffle, ind5_waffle)
                    else:
                        ind3_ind5 = 30
                    ...
                    if ind1_theta != 0:
                        ...
                        ind3_ind1 = davi.subtract_colours_2(
                            ind3_waffle, ind1_waffle)
                    else:
                        ind3_ind1 = 30

                # make comparisons

                optimal_match = {"ind3_ind2": ind3_ind2,
                                 "ind3_ind4": ind3_ind4,
                                 "ind3_ind5": ind3_ind5,
                                 "ind3_ind6": ind3_ind6,
                                 "ind3_ind3": ind3_ind3,
                ...
                if len(nub_match) > 1:
                    ...
                    for iden in identities:
                        if iden == "ind3_ind3":
                            ...
                            distances['ind3_ind3'] = ind2_ind3_distance
                            ...
                            distances['ind3_ind1'] = ind2_ind1_distance
                            ...
                            distances['ind3_ind4'] = ind2_ind4_distance
                            ...
                            distances['ind3_ind5'] = ind2_ind5_distance
                            ...
                            distances['ind3_ind6'] = ind2_ind6_distance
                            ...
                            ind2_ind6_distance = abs(
                                ind2_checker - ind6_checker)
                            distances['ind3_ind2'] = ind2_ind6_distance
                ...
                if true_id == "ind3_ind3":
                    ...
                    if i != mix[-1]:
                        ...
                        for f in range(i, next_elem):
                            ...
                            ant3_copy.loc[f] = test

                    elif i == mix[-1]:
                        continue

                if true_id == "ind3_ind2":
...
ant3_copy.to_hdf("ant3_detections.h5", key="changed_names", format="fixed")
